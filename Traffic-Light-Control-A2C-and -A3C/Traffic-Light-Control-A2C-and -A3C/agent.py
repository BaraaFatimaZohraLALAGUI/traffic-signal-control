...
class A2CAgent:
    ...
    Uses synchronous batch updates for both actor and critic networks.
    """
    def __init__(self):
        self.lane_len = 10
        self.GAMMA = 0.9

        # Experience buffers for batch updates
        self.states = []
        self.rewards = []
        self.actions = []
    ...
    def init_nn(self):
        """
        Build the actor and critic networks for A2C.
        """
        ...
        with tf.name_scope('actornet'):
            ...
            # Manual L2 regularization for TF1.x compatibility
            ...
            reg_variables = [var for var in reg_variables if var.dtype.base_dtype == tf.float32]
            l2_lambda = 1e-6
            reg_term = tf.add_n([tf.nn.l2_loss(var) for var in reg_variables]) * l2_lambda if reg_variables else 0.0
            cost += reg_term
    ...
    def _train(self, advantage, Return, action, state, lr, iter):
        """
        Perform synchronous A2C update for both actor and critic using collected batch.
        """
    ...
    def policy(self, state):
        """
        Sample an action from the current policy (actor network).
        """
    ...
    def value(self, state):
        """
        Estimate the value of a state (critic network).
        """
    ...
    def train(self, state, action, reward, lr, para):
        """
        Store experience and perform A2C update when batch is ready.
        """
